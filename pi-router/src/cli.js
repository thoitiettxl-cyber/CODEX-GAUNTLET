...
export async function runCli(
	...
	if (!["serve", "login", "logout", "models"].includes(parsed.command)) {
	...
	const server = createServer({ runtime, apiKey });
	...
	const displayHost = typeof address === "object" && address?.family === "IPv6"
		? `[${address.address}]`
		: address.address;
	output.write(`pi-router listening on http://${displayHost}:${address.port}\n`);
	return { command: "serve", server, address };
}
